...
export const TransferHistoryModal: React.FC<TransferHistoryModalProps> = ({
  ...
  return (
    <div className="modal-scrim">
      <div className="w-full max-w-xl max-h-[85vh] bg-surface p-6 relative rounded-modal border border-border shadow-modal flex flex-col space-y-4 animate-slide-up">
        {/* Header */}
        <div className="flex items-center justify-between pb-3 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-ink-muted">Local history stored only in this browser</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1 max-h-[60vh]">
          {history.length === 0 ? (
            ...
            history.map((item) => (
              <div
                key={item.id}
                className="p-3 rounded-card bg-canvas-subtle border border-border flex items-center justify-between gap-3 text-xs"
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="space-y-0.5 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-semibold text-ink truncate max-w-[160px] sm:max-w-[240px]">
                        {item.fileName}
                      </span>
